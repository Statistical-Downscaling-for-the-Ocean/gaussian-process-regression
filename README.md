...
Additionally, to install the packages needed to run the examples here:
```
cd /path/to/repo/python # replace /path/to/repo where you cloned this repository
pip install -r requirements.txt
```

The example is a jupyter notebook so, you may also need to start a notebook serve:
```
cd /path/to/repo/python # replace /path/to/repo where you cloned this repository
jupyter lab
```
